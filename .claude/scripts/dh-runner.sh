#!/usr/bin/env bash
# dh-runner.sh — Gradle verification for diet_helper.
# Emits exactly one JSON line on stdout. All gradle/grep noise goes to temp files.
#
# Usage: dh-runner.sh [screenshot_record_needed]
...
# Escape a string for JSON: backslash, doublequote, control chars.
json_escape() {
  local s="$1"
  s="${s//\\/\\\\}"
  s="${s//\"/\\\"}"
...
# Render ERRORS array as JSON array literal.
errors_json() {
  if [ "${#ERRORS[@]}" -eq 0 ]; then
    printf '[]'
    return
...
# Parse "N tests completed, M failed" — gradle prints this on failure only.
...
if [ -n "$SUMMARY_LINE" ]; then
  ...
  # Gradle is silent on green — sum tests from JUnit XML reports.
  TEST_RESULTS_DIR="$REPO_ROOT/app/build/test-results/testDebugUnitTest"
  if [ -d "$TEST_RESULTS_DIR" ]; then
    TOTAL=$(awk -F'"' '/<testsuite / {sum += $4} END {print sum + 0}' "$TEST_RESULTS_DIR"/*.xml 2>/dev/null || echo 0)
    FAILED=0
    TOTAL="${TOTAL:-0}"
    TESTS_RESULT="${TOTAL} passed / 0 failed"
  else
    TESTS_RESULT="no test reports"
    FAILED=1
    add_err "gradle exit=$TEST_EXIT but no JUnit XML reports under $TEST_RESULTS_DIR"
  fi
else
  TESTS_RESULT="no test summary"
  FAILED=1
  # Try to surface a useful error line.
...
# Collect a few FAILED test names if any.
if [ "$FAILED" -gt 0 ]; then
  while IFS= read -r line; do
    [ -n "$line" ] && add_err "$line"
